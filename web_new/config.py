...
class Config():
    ...
    def __init__(self):
        ...
        self.Camera = self.Camera(config)
    ...
    class Blinds:
        ...
        def __init__(self, config):
            self.ports = ast.literal_eval(config["Blinds"]["ports"])
    ...
    class HeatingSensors:
        ...
        def __init__(self, config):
            ...
            self.mapSensorsToManifold = ast.literal_eval(config["HeatingSensors"]["mapSensorsToManifold"])
    ...
    class Lights:
        ...
        def __init__(self, config):
            ...
            self.items = ast.literal_eval(config["Lights"]["items"])
    ...
    class MQTT:
        ...
        def __init__(self, config):
            self.host = config.get("MQTT", "host", fallback="192.168.0.222")
            self.port = int(config.get("MQTT", "port", fallback="443"))
            self.path = config.get("MQTT", "path", fallback="/mqtt")
            self.useSSL = config.get("MQTT", "useSSL", fallback="true").lower() == "true"
    ...
    class Camera:
        ...
        def __init__(self, config):
            self.urls = [
                config.get("Camera", "url1", fallback=""),
                config.get("Camera", "url2", fallback=""),
                config.get("Camera", "url3", fallback=""),
                config.get("Camera", "url4", fallback=""),
            ]
