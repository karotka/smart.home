...
hpTuya = None
_hp_device = conf.Tuya.devices.get("bf06f140ee20807fdaalyq")
if _hp_device:
    hpTuya = tinytuya.OutletDevice(
        dev_id="bf06f140ee20807fdaalyq",
        address=_hp_device["ip"],
        version=_hp_device["ver"]
    )
...
def heating_SensorRefresh(**kwargs):
    ...
    data = dict()
    manifold = utils.toStr(db.get("heating_manifold_state"))
...
def heating(**kwargs):
    ...
    if temperature < conf.Heating.minimalTemperature:
        ...
        temperature = conf.Heating.maximalTemperature
...
class Key(dict):
    ...
    def __lt__(item0, item1):
        ...
        mina = int(a1) * 60 + int(a2)
        minb = int(b1) * 60 + int(b2)
...
def heating_setTemp(**kwargs):
    ...
    if item["temperature"] < conf.Heating.minimalTemperature:
        ...
        item["temperature"] = conf.Heating.maximalTemperature
...
def alarm_status(**kwargs):
    db = conf.db.conn
    zones = {}
    for key in db.keys("alarm_zone_*"):
        zone_id = utils.toStr(key).replace("alarm_zone_", "")
        value = utils.toStr(db.get(key))
        zones[zone_id] = value
    return {
        "zones": zones
    }
